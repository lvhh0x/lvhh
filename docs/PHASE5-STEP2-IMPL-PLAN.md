...
이번 실행 범위는 **박스 조합 알고리즘(엔진②) 재작성 + 마스터 데이터 JSON 분리 + 검증**까지다.
UI 반영(BoxSvg/CompanyResult 표기), 파렛트 빈자리 채우기는 **다음 실행 범위**로 분리한다.
...
| # | 항목 | 결정 |
|---|------|------|
| 1 | 2단계 그리디: 빈자리에 딱 맞는 사이즈가 여러 개일 때 | **잔여가 더 많은 사이즈 우선** |
| 2 | 한 아웃박스에 3개 이상 사이즈 혼합 | **허용** (여러 사이즈 조합해 60단위 정확히 채움) |
| 3 | JSON 분리 범위 | **제품·인박스·아웃박스·파렛트·단위표 전부** (single source of truth) |
...
- 아웃박스 총용량 **60단위**: 145=15, 95=10, 60=6
- 택배박스 총용량 **12단위**: 145=5, 95=3, 60=2

### 2.3 박스 조합 (엔진② — 재작성)
1. **1단계** 제품별로 60단위 풀 아웃박스 추출 → 사이즈별 잔여(<60) 수집
2. **2단계-a** 누적 잔여를 사이즈별로 합쳐 풀 한 번 더 추출(같은 사이즈 여러 행 대비) → 사이즈별 잔여 <60 보장
3. **2단계-b 합침 그리디**: 잔여 많은 사이즈를 seed로, 빈 단위(60−seed)를 *다른 사이즈 덩어리를 통째로* 조합해 **정확히 채울 수 있으면 합쳐 풀 아웃박스**, 아니면 seed 단독 처리
   - 덩어리는 절대 쪼개지 않음 / 3사이즈+ 혼합 허용 / 동률 시 잔여 많은(같으면 사이즈 큰) 우선
4. **단독 처리(finalize)**: 택배 단위 합 ≤12 → (인박스 2개+ → 택배 / 1개 → 낱개), 초과 → 부분 아웃박스
...
- [x] `npx tsc --noEmit` 에러 0 + 검증 스크립트 전부 통과

### 3-6. 검증 로그 문서화 ✅
- [x] `docs/PHASE5-STEP2-VERIFY-LOG.md` 생성 (tsc 결과 + 케이스별 통과 로그) → 다운로드 제공

### 3-7. 푸시 🟦
- [ ] `github:push_files`로 배치 푸시 (master.json/data.ts/outerbox.ts/test/plan/verify-log)
- [ ] 푸시 후 한글 깨짐 여부 재확인
...
- `simulate.ts`·`innerbox.ts`·`weight.ts`·`pallet.ts` 로직 변경 없음 (data.ts export 시그니처 유지로 무영향).
- 푸시 전 반드시 로컬 `tsc` 에러 0 + 검증 통과 확인.

---

## 5. 다음 실행 범위 (이번엔 안 함)
- BoxSvg/CompanyResult에 사이즈·미터·수량 표기 + 글씨 확대
- 엔진③ 파렛트 빈자리 채우기 + SVG
