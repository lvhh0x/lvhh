...
| 현재 단계 | **Phase 5 Step 3 완료 ✅ — Step 3-1~3-5 전부 종결(박스 5열·규격/오버행·슬롯·원단 타입+무게 분리·통합 검증). 통합 회귀 test-step3-5 13/13 + 회귀 4종 무영향 + 실브라우저 5개 항목 육안 확인 완료. 검증: `docs/PHASE5-STEP3-VERIFY-LOG.md`. 다음: Phase 6 — master.json → Supabase DB 연동** |
...
| roll_box_capacities | — | 롤 → 인박스 수용량 (원단별) |
| roll_weights | — | 풀 아웃박스 실측 무게 |
| ribbon_specs | — | 원단별 존재 치수 → 원단 드롭다운 (2026-07-13) |
| ribbon_types | — | 원단 코드명 → 원단 드롭다운 (2026-07-13) |
...
8. **보존 목록** — `app/page.tsx`, `app/stock/**`, `components/stock/**`, `components/home/**`, `components/layout/**`, `lib/stock/**`, `lib/svg/**`, `lib/supabase/**`, `types/backtest.ts`, `types/stock.ts`, `python/**`, `components/svg/PalletSvg.tsx` — 절대 수정 금지
