...
> 참고(부수 사항): `BoxSvg.tsx` 재작성 과정에서 원본이 갖고 있던 한글 문자열의 유니코드 이스케이프(`\uc544\uc6c3\ubc15\uc2a4` 등) 표기가 실제 한글 문자로 바뀜음. 런타임 동작은 완전히 동일하나 불필요한 표기 변경이 섞임 (외과적 수정 원칙 위반, 사용자에게 별도 보고함).
...
IMPL-PLAN Step 2 설계에는 "적재 무게에 택배/낱개 제품무게 누락" 우려가 적혐 있었으나,
...
```
npx tsc --noEmit         → 에러 0 (exit 0)
npm run build            → Compiled successfully (폰트 스턴 로컬 임시, 원복 완료 — push 안 함)
```
...
[신규] scripts/test-step3-3.ts → PASS 80 · FAIL 0
 ...
 S1 700 아웃20        → 필질 20 · overflow=false (경계 OK)
 S2 700 아웃21        → overflow=true (21>20)
 S3 900 아웃30        → 필질 30 · overflow=false
 S4 900 아웃31        → overflow=true (31>30)
 S5 700 아웃18+택배2  → 필질 20 · 층수 5 · 마지막층 4칸 · 높이 1715 · overflow=false
 S6 700 아웃18+택배5  → 필질 23 · overflow=true (23>20)
 S7 700 낱개3         → slotLoose=ceil(3/2)=2 · 필질 2
 S8 700 아웃16+택배2+낱개3 → 필질 20 · overflow=false
 S9 1100 아웃45       → 필질 45 · overflow=false (경계 OK)
...
2-a 잔여 그룹핑 키를 `(size,meter)`→`(size,meter,fabric)`로 바꿔 ①을 구현. ②③는 기존 정확합침·FFD 로직 그대로(원단 섞임 허용).
...
npm run build                        → ✓ 9개 페이지 생성 성공
                                       (폰트 스턴 로컬 임시, git checkout 원복 — push 안 함)
```
