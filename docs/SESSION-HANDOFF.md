...
3. `docs/PROGRESS.md` 를 열어 `## 🔄 Phase 5 Step 3` 섹션을 확인한다 — Step 3-1~3-4는 `[x]` 완료, **Step 3-5만 미완료**.
4. 아래 "3. 다음 세션에서 할 일 — Step 3-5" 섹션의 체크리스트를 그대로 따라간다.
...
Step 3은 5개 하위 단계로 나뉘며, **3-1 ~ 3-4는 완료, 3-5만 남음**.
...
| 3-5 | 통합 검증 (브라우저 육안 확인 + 회귀 테스트 보강) | ⏳ **다음에 할 일** |
...
- **교훈:** 한글(비ASCII) 콘텐츠를 `push_files`로 올릴 때는 push 후 반드시 재클론 → diff 대조까지 해야 안전함. 코드 파일(`.ts`/`.tsx`)은 애초에 `\uXXXX` 이스케이프로 작성했기 때문에 문제가 없었음.

---

## 3. 다음 세션에서 할 일 — Step 3-5 (통합 검증)
...
| `docs/PHASE5-STEP3-VERIFY-LOG.md` | Step 3-1~3-4 전체 검증 로그 (diff·테스트 출력) |
