...
> **상태: 스키마는 2026-07-14 실측 · 보안 현황은 2026-07-29 정정**
> 프로젝트 ID: `lnnxjwfvzaelsoupozke` | ap-northeast-1 (Tokyo) | PG17
> public 스키마 **28개 테이블** · **전부 RLS 활성화, 정책 0개 (전면 차단)**
> ⚠️ 행 수는 2026-07-14 값이라 낡았다 — 현재 수치는 ACP `STATE.md` [TRACK 1]
...
```
현재 상태: public 28개 테이블 전부 RLS 활성화 · 정책 0개 (전면 차단)
           anon 은 읽기도 쓰기도 불가 (customers 185행 -> 0행, INSERT -> 401/42501)
           advisors ERROR 28건 -> 0건
처리 시점: Phase 7 ✅ 완료 2026-07-17 (commit aafdafc)
```

**⚠️ 2026-07-29 정정.** 이 절은 Phase 7이 끝난 뒤로도 *"27개 테이블 전부 RLS 비활성화 ·
위험도 높음"* 이라고 적혀 있었다. **MASTER.md는 매 세션 첫 번째로 읽는 문서라, 여기 남은
낡은 줄은 어떤 코드보다 자주 읽힌다.**

**새 테이블을 만들 때의 규칙:**

- `ENABLE ROW LEVEL SECURITY` 를 걸고 **정책은 만들지 않는다.**
- **공개 SELECT 정책을 절대 추가하지 말 것.** 앱의 모든 읽기는 서버에서 **RLS를 우회하는
  시크릿 키**로 이뤄지므로, 정책을 열면 보호만 잃고 얻는 게 없다.
- ⚠️ 예전의 *"ribbon_types · ribbon_specs 는 SELECT를 열어야 시뮬레이션이 산다"* 는
  **반증됐다**(DB-02 참조). 셋 다 전면 차단 상태에서 시뮬레이션은 정상이다.

**테이블 RLS와 별개로 아직 열려 있는 것:** `archive` 스토리지 버킷은 **공개**라 RLS와
무관하게 공개 URL로 파일이 서빙된다. 대외비 자료는 비공개 버킷 + 인증이 먼저다.
